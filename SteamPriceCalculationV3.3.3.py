...
r3 = (page - 1) * 10    # 动态网页参数变动设置（由于steam市场页面为动态页面，需要加该项内容才可进行换页访问）
params = {'start': str(r3),   # 动态网页翻页设置
          'count': str(r3),
          }
...
def main():
    ...
    time.sleep(0.2)
    # print(urlget_str)
    Surl = urlget_str.replace(
        'https://store.steampowered.com/app/', '').split('/')
    numid = Surl[0]
    ...
    try:
        ...
        start_main()
        return
    url = 'https://steamcommunity.com/market/search?cc=us&q=&category_753_Game%5B0%5D=tag_app_'+str(numid) + \
        '&category_753_cardborder%5B0%5D=tag_cardborder_0&category_753_item_class%5B0%5D=tag_item_class_2&appid=753#p' + \
        str(page)+'_price_desc'
    ...
    if getall == 0:
        global noCardcont
        print('游戏无卡片,重试次数'+str(noCardcont))
        noCardcont += 1
    ...
    i = 1
    for moreCardTimes in range(0, 1 if getall < 10 else int(getall/10) if getall % 10 == 0 else int(getall/10)+1):

        nownum = data.select(".market_listing_row_link")
        for num in range(0, len(nownum)):
            getdata = data.select(
            ...
            for item in getdata:
                ...
                try:
                    f = float(yuan['yuan'].replace(
                        country[countryid], '').replace(',', '.'))
                    # f = float(yuan['yuan'].split(' ')[1].replace(',', '.'))
                except Exception as e:
                    print(Fore.RED+'\n Cookie已过期')
                    start_main()
    ...
    pj_no = pj-(pj*0.13)
    print('平均值'+Fore.YELLOW+str(pj)+Fore.RESET+'去除手续费:'+Fore.YELLOW +
          str(Decimal(pj_no).quantize(Decimal('0.00')))+Fore.RESET+"\n")
    ...
    urldy = 'https://store.steampowered.com/api/appdetails?appids=' + \
        numid+'&filters=price_overview&cc='+country_shop[countryid]
    
    
    try:
        get1 = requests.get(urldy, verify=False, proxies=None,
    ...
    DB2 = float(Decimal(float(final_formatted.replace(
        country[countryid], '').replace(',', '.'))).quantize(Decimal('0.00')))

    overPrice = float(DB1-DB2)
    overPrice = round(overPrice, 2)
